...
`uid` is the device's user-table slot index and is carried as a 16-bit field, so it must be an integer in **1–65534**; `setUser` throws for anything outside that range. It is not a place for application-side identifiers — use `userId` for those. Note this ceiling is separate from the device's record capacity (commonly ~30K users), so both limits apply.
