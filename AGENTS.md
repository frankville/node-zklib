...
- **Multi-group (AND / multi-user) combinations** — a single combination requiring members of several groups (ZKAccess "Verificación Multi-Usuario") — are stored as **concatenated single-digit group numbers** in that slot: groups 2 and 3 → token `"23"` (NOT `"2,3"`). Confirmed on ZEM760 fw 6.60 from a ZKAccess multi-user sync (`CMD_ULG_WRQ` = `1:23::::::::`). `decodeUnlockGroupsInfo` splits each digit into a group and sets `validGroups` to the count (= number of groups that must verify together); the encoder concatenates and **rejects group numbers > 9** (the ASCII form can't disambiguate 2-digit groups). Commas are still tolerated on decode for backward compatibility. Open (not yet captured): per-group user counts > 1 (the "N Usuarios" field), and how a 2-digit group would be encoded. Note the token carries **device** group numbers, which need not match ZKAccess's group names/numbers.
