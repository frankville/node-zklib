...
- **User record layout on a device with no users.** `getUsers()` infers 28B vs 72B by scoring decoded records, so an empty device teaches it nothing and the first write is a guess. Guessing wrong is silent: a ZEM760 (`~SSR = "0"`, stores 28B) **ACKs** a 72-byte write, and the record reads back with an empty password, `role 83` and `enabled false` — the credential does not work, and a reconciliation loop rewrites it every pass because the read-back never matches. `setUser` therefore probes `~SSR` (`1` → 72B SSR, `0` → 28B compact) when neither the caller nor a previous read has settled the layout; devices that do not answer keep the previous default. Confirmed on hardware: written with 28B the same record round-trips intact (`role 0`, `enabled true`, password readable).
  - **The probe is TCP-only, and the empty-table fix makes the UDP gap reachable rather than latent.** That fix landed on *both* transports, so over UDP `getUsers()` now returns `[]` on an empty device and a caller proceeds to write with **no layout detection at all**, at UDP's 28B default — the same bug mirrored (28B onto an SSR panel). Untested: no SSR panel was available. `zklibudp.js` has no equivalent of `userPacketSize` or `detectUserPacketSize`.
  - **`Number(nullish) === 0` has bitten this path three times** — `decodeFreeSizes`' `word()` returns `null` for a field a short reply cannot carry, `getDeviceOption` returns `null` for a reply about another option, and `zk_device.js`'s `GVS<group>` read coerces both to verify style `0`. Compare strictly against `0` on anything decoded from a device reply; a coercing check reads "the device did not say" as "the device said zero".
  - **An option this firmware does not implement is still answered `<name>=`, with an empty value** — confirmed by asking a ZEM760 for an invented `~NoExistOption` on both transports. So an unimplemented option is *attributable* and permanent, while a reply carrying no `=` at all is a stolen or truncated frame rather than a firmware trait. Anything that acts on an option's value should take permanence from a reply the device attributed to the option asked for (`getDeviceOptionResult`), never from the shape of a failure: a mismatch, a frame with no assignment, and a timeout are all transient, and reply-stealing produces the *missing* reply as readily as the wrong one.
  - `getDeviceOption` returns `null` when the reply echoes a *different* option name. It has to: the value is read on the write path to choose the layout, `writeMessage` has no realtime filter, and `~SSR`'s nearest neighbours in the same namespace (`~UserExtFmt`, `~ExtendFmt`) both read `"1"` on this panel — the value that selects the layout that corrupts the record.
  - Unimplemented options answer with an **empty value**, not an error (`MaxUserCount`, `FaceFunOn`, `~MaxUserPhotoCount` all read `""` on the ZEM760), which is why an unreadable `~SSR` degrades to the previous default rather than throwing.
